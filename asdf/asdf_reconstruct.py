...
def generation_atc_config(
    specs,
    reconstruction_codes_dir,
    reconstruction_models_dir=False,
    test_time_training=False,
    sampling_difficulty="easy",
    ...
    if specs["NumAtcParts"] == 1:
        if dataset_name == "rbo":
            ...
            if test_time_training == True:
                model_vec_dic = {}
            names = {
                "0001_000091_-021": [-38, -59],
                "0002_000091_-064": [-43, -16],
                "0003_000064_-021": [-21, -22],
                "0004_000114_-008": [-17, -56],
                "0005_000101_-054": [-44, -25],
                "0006_000091_-062": [-35, -18],
                "0007_000049_-019": [-36, -74],
                "0008_000094_-041": [-27, -31],
                "0009_000085_-038": [-52, -64],
                "0010_000119_-052": [-64, -34],
                "0011_000197_-014": [-34, -51],
                "0012_000040_-051": [-10, -48],
                "0013_000076_-021": [-10, -61],
                "0014_000100_-021": [-34, -47],
                "0016_000122_-030": [-56, -44],
                "0017_000115_-010": [-36, -67],
                "0018_000142_-049": [-83, -30],
                "0019_000092_-052": [-71, -36],
                ...
                for trg in names[name]:
                    ...
                    if test_time_training == True:
                        model_vec_dic[(name[:4], trg)] = os.path.join(
                            os.path.join(
                                reconstruction_models_dir, dataset_name, name + ".pth"
                            )
                        )
                    lat_vec_dic[(name[:4], trg)] = torch.load(
                        os.path.join(
                            reconstruction_codes_dir, dataset_name, name + ".pth"
                        )
                    )
            ...
            if specs["Class"] == "laptop":
                ...
                if sampling_difficulty == "easy":
                    ...
                    angles_mapping = {
                        -36: np.arange(-72, 19)
                    }  # 6-angle dataset animation

            if (
                specs["Class"] == "stapler"
                or specs["Class"] == "washing_machine"
                or specs["Class"] == "door"
                or specs["Class"] == "oven"
            ):
        ...
        for test_ins in test_instances:
            for start in angles_mapping.keys():
                for trg in angles_mapping[start]:
                    if test_time_training == True:
                        model_vec_dic[
                            (test_ins, start[0], start[1], trg[0], trg[1])
                        ] = os.path.join(
                            os.path.join(
                                reconstruction_models_dir,
                                dataset_name,
                                "{:04d}art{:04d}{:04d}.pth".format(
                                    test_ins, start[0], start[1]
                                ),
                            )
                        )
...
def interpolation_atc_config(
    specs, reconstruction_codes_dir, dataset_name="shape2motion"
):
    """
    Modify atc_dic to the angles to test on.
    ...
    if specs["NumAtcParts"] == 1:
        if specs["Class"] == "laptop":
            ...
            angles_mapping = {
                (-72, 18): [13 / 30, 17 / 30, 23 / 30],
                (-54, 0): [2 / 18, 4 / 18, 5 / 18],
                (-36, 18): [5 / 18, 7 / 18, 11 / 18],
                (18, -54): [17 / 24, 15 / 24, 7 / 24],
            }
        #         # extrapolation
        ...
        if (
            ...
            angles_mapping = {
                (90, 0): [13 / 30, 17 / 30, 23 / 30],
                (54, 0): [2 / 18, 4 / 18, 5 / 18],
                (36, 90): [5 / 18, 7 / 18, 11 / 18],
                (18, 90): [17 / 24, 15 / 24, 7 / 24],
            }
            ...
            for start, end in angles_mapping.keys():
                for factor in angles_mapping[(start, end)]:
                    ...
                    if specs["Articulation"] == True:
                        ...
                        atc_vec_dic[(test_ins, start, trg)] = (
                            factor * (atc_end - atc_start) + atc_start
                        )
    ...
    if specs["NumAtcParts"] == 2:
        ...
        if specs["Class"] == "refrigerator":
            ...
            for i, j in angle_mapping.keys():
                angle_mapping_90[(90 - i, 90 - j)] = [
                    90 - angle_mapping[(i, j)][0],
                    90 - angle_mapping[(i, j)][1],
                ]
                ...
                if specs["Articulation"] == True:
                    ...
                    atc_end = np.load(
                        os.path.join(
                            ...
                            dataset_name,
                            "{:04d}art{:04d}{:04d}.npy".format(
                                test_ins, end[0], end[1]
                            ),
                        )
                    )
                    atc_vec_dic[(test_ins, start[0], start[1], trg[0], trg[1])] = (
                        atc_end + atc_start
                    ) / 2
...
def reconstruct(
    ...
    def adjust_learning_rate(
        ...
        if num_iterations < adjust_lr_every * 2:
            ...
            lr = initial_lr * (
                (1 / decreased_by) ** (num_iterations // adjust_lr_every - 2)
            )
    ...
    if infer_with_gt_atc == False:
        if num_atc_parts == 1:
            if specs["Class"] == "laptop":
                ...
                specs["Class"] == "Laptop"
                or specs["Class"] == "Microwave"
                or specs["Class"] == "Stapler"
                or specs["Class"] == "WashingMachine"
            ):
                atc_vec = torch.Tensor([90]).view(1, 1).cuda()
            elif (
                specs["Class"] == "Dishwasher"
                or specs["Class"] == "Oven"
                or specs["Class"] == "Refrigerator"
            ):
                atc_vec = torch.Tensor([45]).view(1, 1).cuda()
            elif specs["Class"] == "Table":
                atc_vec = torch.Tensor([15]).view(1, 1).cuda()  # Roughly 25cm
            else:
                default_val = 90
                print(f"No initial vector defined using {default_val}")
                atc_vec = torch.Tensor([default_val]).view(1, 1).cuda()
        ...
        # atc_optimizer = torch.optim.Adam([atc_vec], lr=lr*100)
    ...
    for e in range(num_iterations * 2):
        ...
        if articulation == True:
            ...
            if infer_with_gt_atc:
                atc_vecs = (
                    sdf_data[1]
                    .view(1, num_atc_parts)
                    .expand(xyz.shape[0], num_atc_parts)
                    .cuda()
                )
            else:
        ...
        if infer_with_gt_atc == False:
            # adjust_learning_rate(lr*100, atc_optimizer, e, decreased_by, adjust_lr_every)
            adjust_learning_rate(
                lr * 1000, atc_optimizer, e, decreased_by, adjust_lr_every
            )
        ...
        lat_vecs = lat_vec.expand(xyz.shape[0], -1)
    ...
    if articulation == True:
        if infer_with_gt_atc:
            ...
            atc_err = (
                torch.mean(torch.abs(atc_vec.detach() - sdf_data[1].cuda()))
                .cpu()
                .data.numpy()
            )
            if return_atc_value:
                atc_err = atc_vec.detach().cpu().data.numpy()
            print(f"{atc_vec = }")
...
def reconstruct_jt(
    ...
    for current_joint_type in ["prismatic", "revolute"]:
        joint_type_one_hot = CARTO.Decoder.utils.encode_joint_types(current_joint_type)[
            0
        ]
        ...
        if current_joint_type == "revolute":
            atc_vec = 50  # Revolute
        elif current_joint_type == "prismatic":
            atc_vec = 15  # Prismatic
        else:
            assert False, "Unknown Joint Type"
        atc_vec = torch.Tensor([atc_vec]).view(1, 1).cuda()
        atc_vec.requires_grad = True
        atc_optimizer = torch.optim.Adam([atc_vec], lr=lr * 1000)
        ...
        for e in range(num_iterations * 2):
            ...
            sdf_data = asdf.data.unpack_sdf_samples_from_ram(
                test_sdf, num_samples, articulation, joint_type=True
            )
            ...
            joint_type_vecs = joint_type_one_hot.expand(
                xyz.shape[0], 2 * num_atc_parts
            ).cuda()
            inputs = torch.cat([lat_vecs, xyz, joint_type_vecs, atc_vecs], 1).cuda()
            pred_sdf = decoder(inputs)
        ...
        if return_atc_value:
            atc_err = atc_vec.detach().cpu().data.numpy()
        print(f"{atc_vec = }")
        # Joint Type Error is `True` when there is an error
        joint_type_err = not torch.all(sdf_data[3] == joint_type_one_hot).cpu().item()
        print(f"{joint_type_err = }")
        current_tuple = (
            loss_num,
            atc_err,
            joint_type_err,
        ...
        if loss_num < best_loss:
            best_loss = loss_num
            best_tuple = copy.deepcopy(current_tuple)
    return best_tuple
...
def reconstruct_ttt(
    ...
    def adjust_learning_rate(
        ...
        if num_iterations < adjust_lr_every * 2:
            ...
            lr = initial_lr * (
                (1 / decreased_by) ** (num_iterations // adjust_lr_every - 4)
            )
    ...
    for idx, (name, param) in enumerate(decoder.named_parameters()):
        ...
        decoder_optimizer = torch.optim.Adam(
            [{"params": filter(lambda p: p.requires_grad, opt_params)}], lr=0.01 * lr
        )
    ...
    if infer_with_gt_atc == False:
        ...
        if num_atc_parts == 2:
            if specs["Class"] == "eyeglasses":
                ...
                raise Exception("Undefined classes")
        atc_vec.requires_grad = True
        # atc_optimizer = torch.optim.Adam([atc_vec], lr=100*lr)
    ...
    for e in range(num_iterations * 3):
        ...
        adjust_learning_rate(lr, lat_optimizer, e, decreased_by, adjust_lr_every)
        adjust_learning_rate(
            0.01 * lr, decoder_optimizer, e, decreased_by, adjust_lr_every
        )
        if infer_with_gt_atc == False:
            # adjust_learning_rate(100*lr, atc_optimizer, e, decreased_by, adjust_lr_every)
            adjust_learning_rate(
                1000 * lr, atc_optimizer, e, decreased_by, adjust_lr_every
            )
...
def reconstruct_testset(
    args, ws, specs, decoder, npz_filenames, saved_model_epoch, dataset_name
):
    ...
    for ii, npz in enumerate(npz_filenames):
        if dataset_name != "ours":
            ...
            if dataset_name == "rbo":
                data_sdf = asdf.data.read_sdf_samples_into_ram_rbo(
                    full_filename,
                    articulation=specs["Articulation"],
                    num_atc_parts=specs["NumAtcParts"],
                )
            else:
                data_sdf = asdf.data.read_sdf_samples_into_ram(
                    full_filename,
                    articulation=specs["Articulation"],
                    num_atc_parts=specs["NumAtcParts"],
                )
            ...
            # Our methd directly iterates over the dataset and "loads into ram"
            data_sdf = npz[0]
            npz_name = npz_filenames.file_ids[ii].stem.split(".")[0]

        mesh_filename = os.path.join(reconstruction_meshes_dir, dataset_name, npz_name)
        latent_filename = os.path.join(
            reconstruction_codes_dir, dataset_name, npz_name + ".pth"
        )
        joint_file_name = os.path.join(reconstruction_codes_dir, dataset_name, npz_name)
        ...
        assert specs["Articulation"] and not args.infer_with_gt_atc

        jt_err = True
        joint_type_vec = None
        start = time.time()
        if specs["Articulation"] == True:
            if not specs["JointTypeInput"]:
                ...
                err, atc_err, jt_err, lat_vec, atc_vec, joint_type_vec = reconstruct_jt(
                    decoder,
                    int(args.iterations),
                    specs["CodeLength"],
                    data_sdf,
        ...
        if specs["Articulation"] == True and args.infer_with_gt_atc == False:
            ...
            err_sum += err
            print(
                "err avg: ", err_sum / (ii + 1), "atc_err avg: ", atc_err_sum / (ii + 1)
            )
        else:
            err_sum += err
        ...
        if specs["Articulation"] == True:
            print(
                "save atc npy: ",
                latent_filename[:-4] + ".npy",
                atc_vec.detach().cpu().numpy(),
            )
            with open(latent_filename[:-4] + ".npy", "wb") as f:
                np.save(f, atc_vec.detach().cpu().numpy())
            with open(joint_file_name + ".error.npy", "w") as f:
                np.savetxt(f, np.expand_dims(atc_err, 0))
            if specs["JointTypeInput"]:
                with open(latent_filename[:-4] + "_type.npy", "wb") as f:
                    np.save(f, joint_type_vec.detach().cpu().numpy())
                with open(joint_file_name + "_type.error.npy", "w") as f:
                    np.savetxt(f, np.expand_dims(jt_err, 0))


def reconstruct_testset_ttt(
    args, ws, specs, decoder, npz_filenames, saved_model_state, dataset_name
):
    # build saving directory
    reconstruction_dir = os.path.join(
        args.experiment_directory,
        ws.recon_testset_ttt_subdir,
        str(saved_model_state["epoch"]),
    )
    ...
    for ii, npz in enumerate(npz_filenames):
        ...
        model_filename = os.path.join(
            reconstruction_models_dir, dataset_name, npz_name + ".pth"
        )
        joint_file_name = os.path.join(reconstruction_codes_dir, dataset_name, npz_name)
...
def generation(
    ...
    gen_meshes_dir = os.path.join(generation_dir, ws.reconstruction_meshes_subdir)
    if not os.path.isdir(gen_meshes_dir):
        os.makedirs(gen_meshes_dir)

    save_latvec_only = False
    num_atc_parts = specs["NumAtcParts"]
    lat_vec_dic, atc_vec_dic = generation_atc_config(
        specs, reconstruction_codes_dir, dataset_name=dataset_name
    )
    ...
    for ii, k in enumerate(lat_vec_dic.keys()):
        ...
        if num_atc_parts == 1:
            if dataset_name == "rbo":
                mesh_filename = os.path.join(
                    gen_meshes_dir, dataset_name, "{}_{:04d}".format(k[0], int(k[1]))
                )
            else:
                mesh_filename = os.path.join(
                    gen_meshes_dir,
                    dataset_name,
                    "{:04d}art{:04d}_lat_from{:04d}to{:04d}".format(
                        k[0], k[1], k[1], k[2]
                    ),
                )
        if num_atc_parts == 2:
            mesh_filename = os.path.join(
                gen_meshes_dir,
                dataset_name,
                "{:04d}art{:04d}{:04d}_lat_from{:04d}{:04d}to{:04d}{:04d}".format(
                    k[0], k[1], k[2], k[1], k[2], k[3], k[4]
                ),
            )
        ...
        if not save_latvec_only:
            ...
            if specs["TrainWithParts"] == True:
                ...
                pos = np.concatenate(
                    [
                        xyz[sdf.view(-1) > 0].cpu().numpy(),
                        sdf[sdf.view(-1) > 0].cpu().numpy().reshape(-1, 1),
                        part_pred[sdf.view(-1) > 0].cpu().numpy().reshape(-1, 1),
                    ],
                    axis=1,
                )
                neg = np.concatenate(
...
def generation_ttt(
    ...
    save_latvec_only = False
    num_atc_parts = specs["NumAtcParts"]
    lat_vec_dic, atc_vec_dic, model_vec_dic = generation_atc_config(
        specs,
        reconstruction_codes_dir,
        reconstruction_models_dir,
...
def interpolate_testset(
    ...
    inter_meshes_dir = os.path.join(inter_dir, ws.reconstruction_meshes_subdir)
